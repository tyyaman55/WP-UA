...
def generate_ai_curated_post(title, extract, available_budget):
    ...
    prompt = (
        ...
        "State what it is plainly and specifically — the odd fact, the strange event, the absurd rule, the improbable coincidence, whatever it is — "
        "using the concrete details already in the background text (names, numbers, dates, causes). "
        "The subject itself should be what's interesting here; don't oversell it with hype language, and don't write it so flatly that the unusual part gets buried in generic description either.\n\n"
        "Rules:\n"
        "- Don't open with \"Imagine\", \"Picture this\", \"What if\", \"Meet\", or similar stock hooks — just start with the fact.\n"
        "- Don't address the reader directly (\"you\"), and don't use hype words like \"insane\", \"wild\", \"you won't believe\".\n"
